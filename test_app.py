...
if page == "수업 소개":
    ...
    <body>
        ...
        <script>
            try {{
                ...
                document.getElementById('map').innerHTML = "<p style='color:red; text-align:center;'>지도를 불러오는 데 실패했습니다.</p>";
            }}
        </script>
    </body>
    </html>
    ...
    try:
        ...
        st.error("지도를 불러올 수 없습니다. 오류가 발생했습니다.")

    # 네이버 지도 바로가기 링크 추가
    naver_maps_url = "https://map.naver.com/v5/search/%EC%A2%85%EB%A1%9C3%EA%B0%80%EC%97%AD%20%ED%8E%B8%EC%9D%98%EC%A0%90"
    st.markdown(
        f'<p style="text-align:center;">오류로 인해 지도 표시가 제한될 경우 '
        f'<a href="{naver_maps_url}" target="_blank" style="color:blue; text-decoration:none;">👉 네이버 지도 바로가기</a></p>',
