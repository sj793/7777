...
if page == "수업 소개":
    ...
    with col2:
        ...
        bars = ax.bar(["총 금액"], [total], color=bar_color)
        ...
        for bar in bars:
            ...
            ax.text(bar.get_x() + bar.get_width() / 2, height + 500, f"{int(height)}원", ha='center', va='bottom')

        # 축 및 스타일 설정
        ax.set_ylim(0, 14000)  # 최대치를 14,000원으로 고정
        ax.set_yticks([0, 5000, 10000, 14000])  # y축 눈금 설정
        ax.set_ylabel("금액 (원)")
        ax.set_title("구매 금액 비교")
        st.pyplot(fig)
