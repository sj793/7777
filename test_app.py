...
if page == "수업 소개":
    ...
    # Google Maps API Key
    google_maps_api_key = "AIzaSyD7lt0KSanRvYmQaJ3CQ0gZGVS-y1urcNI"  # 발급받은 Google Maps API Key로 교체하세요.
        ...
        <script>
            function initMap() {{
                ...
                var mainMarker = new google.maps.Marker({{
                    position: {{ lat: 37.570028, lng: 126.987080 }},
                    map: map,
                    title: "종로3가역"
                }});
                ...
                convenienceStores.forEach(function(store) {{
                    ...
                    var infoWindow = new google.maps.InfoWindow({{
                        content: `<div style='text-align:center; font-size:14px;'>${{store.name}}</div>`
                    }});
    ...
    # Google 지도 바로가기 링크 (명확하게 표시)
    google_maps_url = "https://www.google.com/maps/search/?api=1&query=%EC%A2%85%EB%A1%9C3%EA%B0%80%EC%97%AD+%ED%8E%B8%EC%9D%98%EC%A0%90"
    st.markdown(
        f"""
