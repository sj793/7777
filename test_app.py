...
if page == "수업 소개":
    ...
    with col1:
        ...
        for item, (price, img_name) in items.items():
            with st.container():
                ...
                with sub_col2:
                    if st.checkbox(f"{item} - {price}", key=item):
    ...
    with col2:
        ...
        bars = ax.bar([""], [total], color=bar_color)
        ...
        ax.set_ylim(0, 15000)  # Fix maximum value to 15,000
        ax.set_yticks([0, 5000, 10000, 15000])  # Set y-axis ticks
        st.pyplot(fig)
