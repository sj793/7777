...
if page == "수업 소개":
    ...
    <head>
        ...
        <title>편의점 지도</title>
        <script type="text/javascript" src="https://openapi.map.naver.com/openapi/v3/maps.js?ncpClientId="m16vbixgmx"></script>
        <style>
            #map {{
                width: 100%;
                height: 500px;
    ...
    <body>
        ...
        <script>
            try {{
                ...
                document.getElementById('map').innerHTML = "<div class='error-message'>지도를 불러오는 데 실패했습니다.</div>";
            }}
        </script>
    </body>
    </html>
    ...
    # 네이버 지도 바로가기 링크 (명확하게 표시)
    naver_maps_url = "https://map.naver.com/v5/search/%EC%A2%85%EB%A1%9C3%EA%B0%80%EC%97%AD%20%ED%8E%B8%EC%9D%98%EC%A0%90"
    st.markdown(
        f"""
        <p style="text-align:center; margin-top: 20px;">
            <a href="{naver_maps_url}" target="_blank" 
               style="display:inline-block; padding:10px 20px; background-color:#2db400; color:white; text-decoration:none; 
